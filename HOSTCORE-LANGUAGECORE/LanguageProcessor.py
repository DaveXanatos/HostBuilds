...
def convPostulateQ(statement):
    ...
    m = re.match(r".*\b(?:i'(?:m|ve)|(?:i am|you are|he is|i have|he has|she is|she has)) (?:.*)\s+(?:if|whether)\s+(?P<actualQ>.*)(?: |.|)", statement)
    if m:
...
def disAmbResponse(disAmbig):
    if disAmbig == "what time is it":
        response = "It's <say-as interpret-as='time'>" + str(time.strftime("%I %M %p")) + "</say-as>" # <say-as interpret-as='time'>04:36</say-as>
    elif disAmbig == "what is the date":
        response = "It's <say-as interpret-as='date'>" + str(datetime.date.today().strftime("%A, %B %d")) + "</say-as>"  # <say-as interpret-as='date'>2019-03-05</say-as>
...
def thereIsAre(statement, sentenceParts, fWord, sWord, roSentS):
    ...
    # A huge shaggy is dog running around in the road in the center of town?   --> Same thing with 'shaggy' - it ID's as NN
...
while cnv == 1:
    ...
    #statement = input("> ")  #Here for convenience in case I need it for testing
    ...
    if len(masterResponse) >= 4:
        ...
        if IDKflag == "1":   # I Don't Know flag.  Prevents the IDK response if a response WAS given.
            temp = SpeakText.split("I don't know")
            SpeakText = temp[0]
