# Conversational Postulate Reconfigurer....
...
def convPostulateQ(statement):
    ...
    m = re.match(r".*\b(?:i'(?:m|ve)|(?:i am|you are|he is|i have)) (?:.*)\s+(?:if|whether)\s+(?P<actualQ>.*)(?: |.|)", statement)
    if m:
...
# Run statement through the regular stuff now.  Use cpqFlag in response to add the
# "You have asked me a question in the form of a conversational postulate.  Your actual question is " + statement (which
# will be = to check4CP if cpqFlag was not "0")
